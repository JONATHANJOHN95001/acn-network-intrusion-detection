...
Test size 0.2  |  Kernel PCA (linear)  |  Logistic Regression  (reference sheet #2)

Note: taken in place of sheet #1 Linear Regression, which is a regressor and cannot classify.

Run on its own:

    python Test_0.2_DR_LIN_LR.py

Writes Test_0.2_DR_LIN_LR.jpg (confusion matrix) and appends one row to
results/75_runs.csv.
"""
...
def main():
    ...
    row = {"Test": TEST_SIZE, "DR": "LIN", "Clas": CLASSIFIER,
