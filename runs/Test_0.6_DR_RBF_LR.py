...
Test size 0.6  |  Kernel PCA (rbf)  |  Logistic Regression  (reference sheet #2)

Note: taken in place of sheet #1 Linear Regression, which is a regressor and cannot classify.

Run on its own:

    python Test_0.6_DR_RBF_LR.py

Writes Test_0.6_DR_RBF_LR.jpg (confusion matrix) and appends one row to
results/75_runs.csv.
"""
